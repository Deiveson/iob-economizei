...
export const useHandleTransaction = () => {
    ...
    const { setTransaction } = useTransactionsContext()
    ...
    const sendTransaction = useCallback(
        async ({ ...transaction }: IRegisterTransaction) => {
            const date = new Date().toISOString().substring(0, 10)
            await setTransaction({
                ...transaction,
                date,
                value: Number(transaction.value.replace(/[^0-9,-]+/g, '').replace(/,/, '.')),
            })
            navigate(URIs.Dashboard)
        },
        [navigate, setTransaction],
    )
